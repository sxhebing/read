...
static int av_dict_get_i(FFPlayer *ffp,const char* key){
	AVDictionaryEntry *e = av_dict_get(ffp->player_opts, key,NULL, 0);
    if(e)
	return atoi(e->value);
	else 
	return 0;
...
static void av_dict_get_extra(FFPlayer *ffp,const char* key,DecodeParams *tag){
	AVDictionaryEntry *e = av_dict_get(ffp->player_opts, key,NULL, 0);
    if(e){
		char *line = e->value;
		char* tok = ",";
        char* sub = strtok(line,tok);
		...
		av_log(NULL, AV_LOG_INFO, "av_dict_get_extra with 1 %d",size);
        tag->extra = (unsigned char *)av_malloc(sizeof(unsigned char) * size);
		av_log(NULL, AV_LOG_INFO, "av_dict_get_extra with 2 %d",size);
...
static int copy_from_cache(AVStream* st,AVStream* at,DecodeParams *videoParams,DecodeParams *audioParams){
    /**
    ...
    **/
    ...
    av_log(NULL, AV_LOG_INFO, "copy_from_cache width %d\n",size);
    ...
    for (;; avio_skip(s->pb, 4)) {
        ...
        */
        ...
        if (FLV_TAG_TYPE_META == type) {
            ...
            av_log(NULL, AV_LOG_INFO, "find video flv tag success %d.\n",size);
...
static void find_stream_info(FFPlayer *ffp,AVFormatContext *formatCtx, int64_t start)
{
	DecodeParams *videoParams = (DecodeParams *)av_malloc(sizeof(DecodeParams));
	DecodeParams *audioParams = (DecodeParams *)av_malloc(sizeof(DecodeParams));
	
	av_dict_get_extra(ffp, "video_extra",videoParams);
	videoParams->code_id = (enum AVCodecID)av_dict_get_i(ffp, "video_code_id");
	videoParams->format = av_dict_get_i(ffp, "video_format");
	videoParams->field_order = (enum AVFieldOrder)av_dict_get_i(ffp, "video_field_order");
	videoParams->profile = av_dict_get_i(ffp, "video_profile");
	videoParams->level = av_dict_get_i(ffp, "video_level");
	videoParams->qmin = av_dict_get_i(ffp, "video_qmin");
	videoParams->qmax = av_dict_get_i(ffp, "video_qmax");
	videoParams->width = av_dict_get_i(ffp, "video_width");
	videoParams->height = av_dict_get_i(ffp, "video_height");
	videoParams->aspect.num = av_dict_get_i(ffp, "video_aspect_num");
	videoParams->aspect.den = av_dict_get_i(ffp, "video_aspect_den");
	videoParams->stream_time_base.num = av_dict_get_i(ffp, "video_stream_time_base_num");
	videoParams->stream_time_base.den = av_dict_get_i(ffp, "video_stream_time_base_den");
	videoParams->codec_time_base.num = av_dict_get_i(ffp, "video_codec_time_base_num");
	videoParams->codec_time_base.den = av_dict_get_i(ffp, "video_codec_time_base_den");
	videoParams->r_frame_rate.num = av_dict_get_i(ffp, "video_r_frame_rate_num");
	videoParams->r_frame_rate.den = av_dict_get_i(ffp, "video_r_frame_rate_den");
	videoParams->avg_frame_rate.num = av_dict_get_i(ffp, "video_avg_frame_rate_num");
	videoParams->avg_frame_rate.den = av_dict_get_i(ffp, "video_avg_frame_rate_den");
	videoParams->pts_wrap_bits = av_dict_get_i(ffp, "video_pts_wrap_bits");
	
	av_dict_get_extra(ffp, "audio_extra",audioParams);
	audioParams->code_id = (enum AVCodecID)av_dict_get_i(ffp, "audio_code_id");
	audioParams->format = av_dict_get_i(ffp, "audio_format");
	audioParams->profile = av_dict_get_i(ffp, "audio_profile");
	audioParams->frame_size = av_dict_get_i(ffp, "audio_frame_size");
	audioParams->bits_per_coded_sample = av_dict_get_i(ffp, "audio_bits_per_coded_sample");
	audioParams->sample_rate = av_dict_get_i(ffp, "audio_sample_rate");
	audioParams->channels = av_dict_get_i(ffp, "audio_channels");
	audioParams->channel_layout = av_dict_get_i(ffp, "audio_channel_layout");
	audioParams->stream_time_base.num = av_dict_get_i(ffp, "audio_stream_time_base_num");
	audioParams->stream_time_base.den = av_dict_get_i(ffp, "audio_stream_time_base_den");
	audioParams->codec_time_base.num = av_dict_get_i(ffp, "audio_codec_time_base_num");
	audioParams->codec_time_base.den = av_dict_get_i(ffp, "audio_codec_time_base_den");
	audioParams->pkt_timebase.num = av_dict_get_i(ffp, "audio_pkt_timebase_num");
	audioParams->pkt_timebase.den = av_dict_get_i(ffp, "audio_pkt_timebase_den");
	audioParams->pts_wrap_bits = av_dict_get_i(ffp, "audio_pts_wrap_bits");
    ...
    **/
    ...
    st->codec->pix_fmt = (enum AVPixelFormat)videoParams->format;//AV_PIX_FMT_YUV420P;
    ...
    //at->codec->refs = 1;
    at->codec->sample_fmt = (enum AVSampleFormat)audioParams->format;
    ...
    av_log(NULL, AV_LOG_INFO, "==========>begin find get_video_extradata: %0.3fs\n",(av_gettime()-start) / 1000000.0 );
    if(1){
        copy_from_cache(st,at,videoParams,audioParams);
    }else{
        av_log(NULL, AV_LOG_INFO, "get_video_extradata ret = %d\n",get_video_extradata(formatCtx, 0));
    }
    av_log(NULL, AV_LOG_INFO, "==========>after find get_video_extradata: %0.3fs\n",(av_gettime()-start) / 1000000.0 );
    formatCtx->pb->buf_ptr = formatCtx->pb->buf_end;
    formatCtx->pb->pos = (int64_t) formatCtx->pb->buf_end;
}

//add by hebing 2017/6/28 end
...
static void control_audio_queue_duration(FFPlayer *ffp, VideoState *is) {
    ...
    if (cached_duration > is->max_cached_duration) {
        // drop
        av_log(NULL, AV_LOG_INFO, "audio cached_duration = %lld, nb_packets = %d.\n", cached_duration, nb_packets);
...
static void video_image_display2(FFPlayer *ffp)
{
    ...
    if (vp->bmp) {
	    ...
	    //add for
	    if(ffp->blive == -1){
		long duration_l = ffp_get_duration_l(ffp);
		if(duration_l == 0){
		    ffp->blive = 1;
        	...
        	av_log(NULL,AV_LOG_INFO,"blive = %d\n",ffp->blive);
	    }
#endif
...
static int read_thread(void *arg)
{
	...
	//for rtmp can load from config!
	if (av_stristart(is->filename, "rtmp", NULL) && av_dict_get_i(ffp,"from_config"))
		find_stream_info(ffp, ic, av_gettime());
    else
	...
	if(max_cached_duration <= 0){
	    ...
	    av_log(NULL,AV_LOG_INFO,"set max_cached_duration = %d",max_cached_duration);
	}
    }else{
	is->max_cached_duration = 0;
    }
...
#if MAX_CACHE_ENABLE
	if(!is->paused && ffp->blive && ffp->first_video_frame_rendered && is->max_cached_duration > 0 && (pkt->flags & AV_PKT_FLAG_KEY)){
...
void ffp_check_buffering_l(FFPlayer *ffp)
{
	...
	if(1){
	    //add for live
	    hwm_in_ms = hwm_in_ms > 3000 ? 3000 : hwm_in_ms;
	}
