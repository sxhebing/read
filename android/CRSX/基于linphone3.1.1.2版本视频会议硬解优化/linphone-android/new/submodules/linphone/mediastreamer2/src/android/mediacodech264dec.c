	...
	*new_sps_pps=FALSE;
	...
	while((im=ms_queue_get(naluq))!=NULL){
		...
		if (dst+nal_len+100>end){
			...
			ms_error("Call enlarge_bitstream %p & %x ",d->bitstream,d->bitstream_size);
		}
			...
			if(oBufidx >= 0){
				...
				if(!d->first_image_decoded){
					...
					if(format != NULL){
						...
						d->color = color;
						AMediaFormat_delete(format);
						ms_error("Call AMediaCodec_getOutputFormat %ix%i %i",width,height,color);
					}
				}else{
					width = d->vsize.width;
					height = d->vsize.height;
					color = d->color;
				}
			//}
			...
			//if(buf != NULL){
				...
				//if(width != 0 && height != 0 ){
					if(color == 19) {
						...
						om = copy_ycbcrbiplanar_to_true_yuv_with_rotation_and_down_scale_by_2(d->buf_allocator, buf, cbcr_src, 0, width, height, width, width, FALSE, FALSE);
					}
